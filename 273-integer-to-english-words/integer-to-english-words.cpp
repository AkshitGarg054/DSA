...
public:
    ...
    string solve(int num) {
        ...
        if(num < 100) return belowHundred[num / 10] + ((num % 10 != 0) ? " " + belowTen[num % 10] : ""); // 89
        if(num < 1000) return belowTen[num / 100] + " Hundred" + ((num % 100 != 0) ? " " + solve(num % 100) : ""); // 879
        if(num < 1000000) return solve(num / 1000) + " Thousand" + ((num % 1000 != 0) ? " " + solve(num % 1000) : ""); // 28,456
        if(num < 1000000000) return solve(num / 1000000) + " Million" + ((num % 1000000 != 0) ? " " + solve(num % 1000000) : ""); // 1234567
